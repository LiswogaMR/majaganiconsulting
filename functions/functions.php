    ...
    switch($actionType){
        ...
        case 'addUser':
            ...
            if(count($result) > 0){
                ...
                VALUES ('".$name."', '".$surname."','".$password."','".$email."','". $dateTime."','". $status."')";
                if (mysqli_query($conn,$sql )) {
        ...
        case 'postJob': //I will update job posted here.
            ...
            $applicationDeadline = mysqli_real_escape_string($conn, $_POST['applicationDeadline']);
            $status = mysqli_real_escape_string($conn, $_POST['status']);
            ...
            $postedBy = "admin_user"; // Replace with the dynamic user data, if available

            // SQL query to insert data into the database
            $sql = "INSERT INTO job_postings (job_title, department, job_description, job_requirements, job_type, application_deadline, status, posted_by, jobLoc, jobAdd)
                    VALUES ('$jobTitle', '$department', '$jobDescription', '$jobRequirements', '$jobType', '$applicationDeadline', '$status', '$loggedInUserEmail' , '$jobLoc' , '$jobAdd')";

            // Execute the query
            if ($conn->query($sql) === TRUE) {
                $_SESSION['msg'] = "New job posting created successfully";
                // echo "New job posting created successfully";
                // Optionally, redirect or display a success message
                header("Location: ../employee.php"); // Replace with your success page
                exit();
            } else {
        ...
        case 'updatepostJob':
            ...
            $sql = "UPDATE job_postings 
                    SET job_title = '$jobTitle', department = '$department', job_description = '$jobDescription', 
                        job_requirements = '$jobRequirements', job_type = '$jobType', 
                        application_deadline = '$applicationDeadline', status = '$status', posted_by = '$postedBy'
                        , jobLoc = '$jobLoc', jobAdd = '$jobAdd' 
                    WHERE id = $jobId";

            if (mysqli_query($conn, $sql)) {
                $_SESSION['msg'] = "Job updated successfully.";
            } else {
                $_SESSION['msg'] = "Error updating job: " . mysqli_error($conn);
            }
